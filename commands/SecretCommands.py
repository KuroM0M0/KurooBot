...
class SecretCommands(commands.Cog):
    ...
    async def verkraben(self, ctx, member: discord.Member = None):
        ...
        if member == None:
            await ctx.send(f"{target.mention} hat sich verkraben! <:Schaufel:1430552039456510117>")
        else:
            await ctx.send(f"{target.mention} geh dich verkraben! <:Schaufel:1430552039456510117>")
